...
const NS = 'org.acme.watch.auction';
...
describe('CarAuction', () => {
    ...
    describe('#makeOffer', () => {
        ...
        it('should add the offer to the offers of a watch listing', async () => {
            ...
            // create the watch
            const watch = factory.newResource(NS, 'Watch', 'CAR_001');
            watch.owner = factory.newRelationship(NS, 'Member', seller.$identifier);

            // create the watch listing
            const listing = factory.newResource(NS, 'WatchListing', 'LISTING_001');
            listing.reservePrice = 100;
            listing.description = 'My nice car';
            listing.state = 'FOR_SALE';
            listing.watch = factory.newRelationship(NS, 'Watch', 'CAR_001');
            ...
            offer.listing = factory.newRelationship(NS, 'WatchListing', 'LISTING_001');
            offer.bidPrice = 200;

            // Get the registries.
            const watchRegistry = await businessNetworkConnection.getAssetRegistry(NS + '.Watch');
            const watchListingRegistry = await businessNetworkConnection.getAssetRegistry(NS + '.WatchListing');
            ...
            // Add the Watch to the asset registry.
            await watchRegistry.add(watch);

            // Add the WatchListing to the asset registry
            await watchListingRegistry.add(listing);
            ...
            lowOffer.listing = factory.newRelationship(NS, 'WatchListing', 'LISTING_001');
            lowOffer.bidPrice = 50;
            await businessNetworkConnection.submitTransaction(lowOffer);

            // get the listing
            let newListing = await watchListingRegistry.get(listing.$identifier);
            ...
            closeBidding.listing = factory.newRelationship(NS, 'WatchListing', 'LISTING_001');
            await businessNetworkConnection.submitTransaction(closeBidding);

            // get the listing
            newListing = await watchListingRegistry.get(listing.$identifier);
            ...
            // get the watch
            const theWatch = await watchRegistry.get(watch.$identifier);

            // check that the buyer now owns the car
            theWatch.owner.getIdentifier().should.equal(buyer.$identifier);
        });
        ...
        describe('#closeBidding', () => {
            it('with no bids should result in RESERVE_NOT_MET', async () => {
                ...
                listing.description = 'My nice car';
                listing.state = 'FOR_SALE';
                listing.watch = factory.newRelationship(NS, 'Watch', watch.$identifier);
                ...
                // add the watch listing
                await watchListingRegistry.add(listing);

                // close the bidding
                const closeBidding = factory.newTransaction(NS, 'CloseBidding');
                closeBidding.listing = factory.newRelationship(NS, 'WatchListing', listing.$identifier);
                await businessNetworkConnection.submitTransaction(closeBidding);

                // get the listing and check state
                const watchListing = await watchListingRegistry.get(listing.$identifier);
                watchListing.state.should.equal('RESERVE_NOT_MET');
            });
        });
    });
});
